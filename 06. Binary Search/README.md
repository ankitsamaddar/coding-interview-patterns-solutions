...
1. [Find the Insertion Index](<01-find-the-insertion-index.md>) :
...
3. [Search in Rotated Sorted Array](<03-search-in-rotated-sorted-array.md>) : Given an array of unique integers sorted in ascending order that has been rotated at an unknown pivot, write a function to search for a target value. If the target exists, return its index. If not, return -1.
...
7. [Find the Median From Two Sorted Arrays](<07-find-the-median-from-two-sorted-arrays.md>) : You are given two sorted integer arrays. Your task is to find their median value as if they were merged into a single sorted sequence. The overall run time complexity should be $O(\log (m+n))$.
